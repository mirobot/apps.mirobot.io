function MirobotRunner(mirobot) {
  ...
  this.forward = function (distance, el) {
    this.stack.push(
      runCmd(function () {
        self.mirobot.forward(distance, done());
      }, el)
    );
  };
  this.back = function (distance, el) {
    this.stack.push(
      runCmd(function () {
        self.mirobot.back(distance, done());
      }, el)
    );
  };
  ...
  this.beep = function (duration, el) {
    this.stack.push(
      runCmd(function () {
        self.mirobot.beep(duration, done());
      }, el)
    );
  };
}
...
MirobotBlockly.prototype = {
  init: function (id) {
    ...
    this.workspace = Blockly.inject(this.editorId, {
      ...
      media: "/apps.mirobot.io/assets/apps/blockly/media/",
    });
  ...
  initControls: function (id) {
    ...
    this.controls.querySelector(".run").addEventListener("click", function () {
      self.clickRun();
    });
    this.controls
      .querySelector(".pause")
  ...
  setRunButtonState: function (state) {
    this.controls.querySelector(".run").style.display =
      state === "run" ? "" : "none";
    this.controls.querySelector(".pause").style.display =
      state === "pause" ? "" : "none";
  },
  clickRun: function () {
  ...
  setMirobot: function (mirobot) {
    ...
    this.mirobot.addEventListener("programComplete", function (msg) {
      self.completeHandler(msg);
    });
    this.mirobot.addEventListener("readyStateChange", function () {
      self.updateMirobotState();
    });
